...
(Any response along these lines was accepted)
```
...
#check to see if that worked using the unique function on the column of interest
unique(lakersQ2$team) #REPLACE [FILL ME IN] WITH YOUR RESPONSE
```
...
```{r, eval = FALSE}
#WRITE YOUR ANSWER TO THE ABOVE QUESTION HERE AND SCREENSHOT
Kobe, Pau, and Lamar Odom missed the most free throws. Again, these are the stars of the team, so they probably had more opportunity to take free throws. A fairer metric of free throw performance would be to diving the number of missed free throws by the total number of free throws taken.
```
...
We cannot accomplish this task for assists, because there is no data recorded on assists in the etype column. We will have to go looking for that data elsewhere to answer this question.
```
...
```{r, eval =  FALSE}
#WRITE YOUR ANSWER TO THE ABOVE QUESTION HERE AND SCREENSHOT
Any answer that took it a step beyond the face value of these data and deviated from the examples I provided above was accepted.
```
